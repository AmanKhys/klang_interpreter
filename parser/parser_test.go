...
func TestIdentiferExpression(t *testing.T) {
	...
	if ident.Value != "foobar" {
		t.Errorf("ident.Valuye not %s. got = %s", "foobar", ident.Value)
	}
...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("program.Satements[0] is not ast.ExpressionStatement. got = %T", stmt)
	}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("program.Satements does not contain %d statements. got = %d\n", 1, len(program.Statements))
		}
		stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
			t.Fatalf("program.Statements[0] is not ast.ExpressionStatement. got = %T", program.Statements[0])
...
func testIntegerLiteral(t *testing.T, il ast.Expression, value int64) bool {
	...
	if !ok {
		t.Errorf("il not *ast.IngeterLIteral. got = %T", il)
		return false
	}
	if i.Value != value {
